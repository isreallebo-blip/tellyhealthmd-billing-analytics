CREATE OR REPLACE FUNCTION public.get_dashboard_stats(
  _companies text[] DEFAULT NULL::text[],
  _providers text[] DEFAULT NULL::text[],
  _insurances text[] DEFAULT NULL::text[],
...
BEGIN
  WITH filtered AS (
    ...
    JOIN public.source_files sf ON sf.id = c.source_file_id
    WHERE sf.status = 'approved'
      AND (_companies  IS NULL OR array_length(_companies,1)  IS NULL OR c.company         = ANY(_companies))
